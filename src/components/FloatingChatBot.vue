  ...
  <div class="fixed bottom-6 right-6 z-[100] flex flex-col items-end">
    ...
    >
      <div v-if="isOpen" class="mb-4 w-[350px] sm:w-[400px] h-[500px] bg-white rounded-[2rem] shadow-2xl border border-slate-100 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4">
        ...
        <!-- Chat Header -->
        <div class="p-6 bg-slate-900 text-white flex items-center justify-between shadow-lg">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-2xl bg-primary flex items-center justify-center shadow-lg shadow-primary/20">
              <Bot class="w-6 h-6 text-white" />
            </div>
            <div>
              <h3 class="font-black text-sm tracking-tight text-white m-0 leading-none">Portal Assistant</h3>
          ...
          <div v-for="msg in messages" :key="msg.id" :class="['flex w-full', msg.role === 'bot' ? 'justify-start' : 'justify-end']">
            <div :class="['flex max-w-[85%] gap-2', msg.role === 'user' ? 'flex-row-reverse' : '']">
              ...
              <div class="space-y-1">
                <div 
                  :class="[
                    'p-4 rounded-[1.5rem] text-sm leading-relaxed font-medium',
                    msg.role === 'bot' 
                ...
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest px-1">
                  {{ msg.time }}
                </p>
              </div>
            </div>
        ...
        <div class="p-6 bg-white border-t border-slate-100">
          <!-- Quick Suggestions -->
          <div v-if="messages.length < 3" class="flex flex-wrap gap-2 mb-4">
             <button 
